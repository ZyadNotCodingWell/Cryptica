...
export default function DashHeader({
  ...
  return (
    <section className="w-full rounded-2xl py-4 px-8 gap-8 flex items-center justify-between bg-neutral-950 backdrop-blur-3xl border border-neutral-300/5 transition-all duration-300 z-50">
      <div className="text-neutral-300/50 text-sm w-full">Welcome back <span className="text-lime-500">{username}</span></div>


      <div className="relative w-full max-w-md backdrop-blur-lg rounded-lg">
        <form
          onSubmit={(e) => e.preventDefault()}
          className={
            showResults
              ? "flex flex-row w-full items-center justify-between py-2 bg-neutral-900/30 rounded-lg  backdrop-blur-md border border-t-0 border-l-0 border-r-2 border-transparent transition duration-0 focus-within:border-b-0 focus-within:hover:border-b-0"
              : "flex flex-row w-full items-center justify-between py-2 bg-neutral-900/30 rounded-lg  border border-neutral-300/5 transition"
          }
        >
          <input
            type="text"
            ...
            className="bg-transparent text-neutral-300/50 placeholder:text-neutral-300/50 flex w-full ring-transparent outline-none px-4"
          />
        </form>

        {/* Render parent-passed results */}
				<ul
					  className={`border border-neutral-300/5 border-t-0 absolute top-full px-1 w-full max-h-64 overflow-y-auto bg-neutral-950 rounded-b-lg shadow-lg scrollbar-thin scrollbar-track-neutral-950 scrollbar-thumb-neutral-800 transition-opacity duration-200 ${
			...
			<div onClick={() => setSettingsOpen(!settingsOpen)} onMouseLeave={() => setSettingsOpen(false)} className={settingsOpen ? `flex flex-row-reverse items-center w-full h-full gap-4` :`flex flex-row-reverse items-center w-full h-full gap-4`}>
      	<div className={settingsOpen? "text-xl w-fit text-neutral-300/50" : "text-neutral-300/30 text-xl w-fit hover:text-neutral-300/50 transition cursor-pointer"}>
				...
				<div className={settingsOpen ? "flex flex-row-reverse gap-4" : "hidden"} >
					<Link href="/">
						<div className="text-neutral-300/50 hover:text-red-600 transition size-6">
							<svg fill="currentColor" viewBox="0 0 1024 1024" xmlns="http://www.w3.org/2000/svg" className="icon">
					...
					<Link href="/">
						<div className="text-neutral-300/50 hover:text-yellow-600 transition size-6">
