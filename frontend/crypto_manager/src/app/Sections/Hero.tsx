		...
		<section className="max-w-[100vw]">
            ...
            <div key={item.id} className={(item.id === 0 || (item.value >= candlebars[item.id - 1].value))? "bg-lime-500 w-2 rounded-full [mask-image:linear-gradient(to_top,transparent_15%,black_25%,black_75%,transparent_85%)]":"bg-red-500 w-2 rounded-full [mask-image:linear-gradient(to_top,transparent_25%,black_35%,black_75%,transparent_85%)]"} style={{ height: `${item.value * 5}%`}}></div>
				...
				<div className="w-full flex flex-col items-center justify-center py-4">
					<div className="text-center font-mono text-transparent bg-gradient-to-t pb-2 from-neutral-200/80 to-neutral-200/95 bg-clip-text text-8xl text-balance cursor-default">Crypto forecasting and tracking</div>
					<div className="text-center text-3xl text-neutral-50/50 max-w-3xl text-balance mt-4 font-light cursor-default">We developed an app to visualize the crypto market while being able to make forecasts and predictions.</div>
